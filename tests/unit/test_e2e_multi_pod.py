...
def test_render_produces_a_service_then_an_indexed_job():
    """A run is deployed as a headless Service plus an Indexed Job."""
...
def test_render_job_runs_every_pod_together_and_never_retries():
    """Every pod runs concurrently, and a failed pod is never retried."""
...
def test_render_gives_every_pod_its_own_shared_memory():
    """Each pod gets private shared memory, so vLLM reads them as distinct hosts."""
...
def test_render_points_the_store_host_at_pod_zero():
    """Pod 0 is the single address known in advance; peers are addressable by index."""
    spec = _job_spec()
...
def test_render_passes_pod_env_through_to_the_runner():
    """Caller-supplied environment reaches the in-pod runner."""
...
def test_render_publishes_the_pod_ip_through_the_downward_api():
    """Each pod learns its own address from the cluster."""
...
def test_render_env_supplies_the_arbitrary_uid_defaults():
    """An arbitrary-UID run gets what it needs to import vLLM and load weights."""
    environment = {
...
def test_render_env_lets_the_caller_override_a_default():
    """Cluster-specific values take precedence over the built-in container defaults."""
    environment = {
...
def test_bootstrap_creates_the_writable_directories():
    """Every writable path exists before the runner starts."""
...
def test_render_gives_identical_argv_to_every_pod():
    """Every pod receives the same argv; identity never appears in it."""
...
def test_render_unpacks_a_source_overlay_over_the_image_copy():
    """An overlay lets a run use newer code than the image holds."""
...
def test_render_without_an_overlay_runs_from_the_image_app_dir():
    """Without an overlay the run uses the image's own copy unchanged."""
    container = manifest.render_container(_job_spec())
...
def test_render_can_require_every_pod_on_a_different_node():
    """Placement can force pods onto separate nodes and away from named ones."""
...
def test_render_can_require_every_pod_on_the_same_node():
    """Placement can force every pod onto a single node."""
...
def test_render_rejects_contradictory_placement():
    """Contradictory placement requirements are rejected, not silently ordered."""
...
def test_render_omits_affinity_when_placement_is_unconstrained():
    """Unconstrained placement imposes no scheduling restrictions."""
