...
routed experts, so the AFD scenarios shard those experts across four FFN
ranks at ``--tensor-parallel-size 1`` with expert parallelism, by data
parallelism rather than by TP. The adapter rejects ``tensor_parallel_size > 1``
on both roles, so TP is never a capacity lever here.

The topology is the balanced ``4a4f`` rather than an FFN-skewed ``1a4f``.
``P2pNcclAFDConnector`` orders its world as ``[F0, F1, ..., A0, A1, ...]`` and
gives every FFN rank a subgroup of itself plus one or more consecutive
Attention ranks, which requires ``num_attention_ranks >= num_ffn_ranks``, so an
FFN-skewed split is rejected outright by ``validate_p2p_topology``.

Four is the smallest balanced rank count that also works: the FFN rank count
must divide ``n_routed_experts``. ``InklingMoE`` pads its FusedMoE expert count
up to a multiple of the EP size, because the TRTLLM kernels assume equal
contiguous per-rank slabs. With 256 routed experts an EP size of 3 pads to 258
and hands the last rank global expert ids 256 and 257, which the checkpoint
does not contain -- weight loading then dies in ``load_expert_weight`` with
``IndexError: index 256 is out of bounds``. EP sizes 1, 2, 4 and 8 pad to 256
and load cleanly; 3, 5 and 6 do not.
...
This suite requires a single node with at least eight 80 GB GPUs and is not
part of the ``test-ready`` gate, which runs on four L4s. ``afd-graph-4a4f`` and
``afd-graph-dbo-4a4f`` exercise configurations that have no prior Inkling
