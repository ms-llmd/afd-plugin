...
| GPU | `P2pNcclAFDConnector` | Inkling-Small | Decode | Experimental. `4a4f` is the only topology: both scripts have served GSM8K through the AFD boundary on 8x H100-80GB, but the E2E gate still fails on a teardown hang. Attention and FFN TP are fixed at 1 and `--language-model-only` is mandatory | [Launch examples](gpu/P2pNcclAFDConnector/inkling_small/README.md) |
