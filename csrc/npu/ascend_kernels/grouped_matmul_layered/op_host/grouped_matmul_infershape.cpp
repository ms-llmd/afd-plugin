...
static ge::graphStatus GetGroupSize(const gert::InferShapeContext* context, GMMParamsInfo& paramsInfo) {
    ...
    // layered: every all_weight element is a single full-expert tensor (e,k,n), so the
    // expert count is weight[0]'s first dim, never the tensor-list length (= layer count).
    const gert::Shape* w0Shape = context->GetDynamicInputShape(GMM_INDEX_IN_WEIGHT, 0);
    OP_CHECK_NULL_WITH_CONTEXT(context, w0Shape);
    OP_CHECK_IF(w0Shape->GetDimNum() != GMM_SPLIT_M_SINGLE_WEIGHT_DIM,
              OP_LOGE(context->GetNodeName(), "all_weight[0] must be a 3-dim (e, k, n) per-layer tensor, "
                        "but now dim num is %zu.", w0Shape->GetDimNum()),
              return GRAPH_FAILED);
    groupNum = static_cast<size_t>(w0Shape->GetDim(0));
...
static ge::graphStatus CheckOptionalTensorList(gert::InferShapeContext* context, const std::string tensorType,
    ...
    // layered: every all_* element is a per-layer full-expert single tensor, so
    // the single-weight branch applies regardless of the layer count.
    bool isSingleWeight = true;
    ...
    // layered: validate every layer's tensor against its own weight element (all
    // layers are shape-identical by contract; the loop keeps per-layer diagnostics).
    ...
    for (size_t layerIdx = 0; layerIdx < static_cast<size_t>(weightGroupedSize); ++layerIdx) {
        auto tensorShape = context->GetDynamicInputShape(nodeIdx, layerIdx);
        OP_CHECK_IF(tensorShape == nullptr, OP_LOGE(context->GetNodeName(),
                  "%s[%lu] must not be nullptr, but now is nullptr.", tensorType.c_str(), layerIdx),
                  return GRAPH_FAILED);
        auto wShape = context->GetDynamicInputShape(GMM_INDEX_IN_WEIGHT, layerIdx);
        ...
        OP_CHECK_IF(CheckDimNumAndPerGroupNum(context, isAntiquantInt4, {tensorDimNum, 3, k}, tensorShape, tensorType) != GRAPH_SUCCESS,
                  OP_LOGE(context->GetNodeName(), "CheckDimNumAndPerGroupNum failed."), return GRAPH_FAILED);
        OP_CHECK_IF(static_cast<size_t>(tensorShape->GetDim(0)) != groupNum, OP_LOGE(context->GetNodeName(),
                  "%s[%zu] batch size[%ld] should be equal with expert count[%lu].", tensorType.c_str(), layerIdx,
                  tensorShape->GetDim(0), groupNum), return GRAPH_FAILED);
        // tensor's N axis size should equal with the same layer weight's N axis.
        ...
        OP_CHECK_IF(tensorNDimValue != weightNDimValue, OP_LOGE(context->GetNodeName(),
                  "NDim[%ld] of %s[%zu] should be equal with NDim[%ld] of weight[%zu].",
                  tensorNDimValue, tensorType.c_str(), layerIdx, weightNDimValue, layerIdx), return GRAPH_FAILED);
    }
    return GRAPH_SUCCESS;
}
...
static ge::graphStatus CheckGroupedMatmulQuant(gert::InferShapeContext* context, const GMMAttrs& gmmAttrs,
    ...
    OP_CHECK_IF(gmmAttrs.groupType == GMM_SPLIT_K,
              OP_LOGE(context->GetNodeName(), "quant cases do not support split axis is K."),
              return GRAPH_FAILED);
    OP_CHECK_IF(!IsTensorListNullOrEmpty(context, GMM_INDEX_IN_OFFSET),
...
static ge::graphStatus CheckGroupedMatmulAntiQuantForShape(gert::InferShapeContext* context, const GMMAttrs& gmmAttrs, const GMMParamsInfo& paramsInfo) {
    ...
    OP_CHECK_IF(gmmAttrs.groupType == GMM_SPLIT_K, OP_LOGE(context->GetNodeName(), "antiquant cases do not support split axis is K."),
              return GRAPH_FAILED);
    OP_CHECK_IF(IsTensorListNullOrEmpty(context, GMM_INDEX_IN_ANTIQUANT_SCALE),
...
static ge::graphStatus CheckCaseNoSplit(gert::InferShapeContext* context, bool transposeWeight,
    ...
    for (size_t i = 0; i < xSize; i++) {
        ...
        OP_CHECK_IF(xKDimValue != weightKDimValue,
                  ...
                  return GRAPH_FAILED);
        // if weight is not transposed, check N axis; otherwise, check K axis, which can be skipped
        if (!(ret == GRAPH_SUCCESS && GmmDavidSupportSoc.count(platformInfo.str_info.short_soc_version) > 0)) {
...
static ge::graphStatus CheckShapeSameLengthTensorList(gert::InferShapeContext* context,
    ...
    OP_CHECK_IF(TensorType2NodeId(tensorType, nodeIdx) != GRAPH_SUCCESS,
              ...
              return GRAPH_FAILED);
    // check two tensorlist's size to be the same, and tensors to have consistent dimension.
    const gert::Shape* shape;
...
// layered: CheckShapeDiffLengthTensorList was removed with the separated-weight
// split-m checkers (its only callers); the layered op has no diff-length case.
...
static ge::graphStatus SplitMSingleXSingleWeightSingleY(gert::InferShapeContext* context, bool transposeWeight,
    ...
    OP_CHECK_IF(CheckDimNum(context, paramsInfo.numX, GMM_MIN_FM_DIM, "x") != GRAPH_SUCCESS,
              ...
              return GRAPH_FAILED);
    // layered: one weight element per layer, each a full-expert (e, k, n) tensor.
    OP_CHECK_IF(CheckDimNum(context, paramsInfo.numWeight, GMM_SPLIT_M_SINGLE_WEIGHT_DIM, "weight") != GRAPH_SUCCESS,
              ...
              return GRAPH_FAILED);
    // check shape, x(m,k), weight(e,k,n), y(m,n)
    // layered: every x element (single or per-group) aligns with the layer weight's
    // K axis - the x list is not parallel to the weight (layer) list.
    ...
    const gert::Shape* w0Shape = context->GetDynamicInputShape(GMM_INDEX_IN_WEIGHT, 0);
    OP_CHECK_NULL_WITH_CONTEXT(context, w0Shape);
    int64_t weightKDimValue = w0Shape->GetDim(kAxisOfWeight);
    for (size_t i = 0; i < paramsInfo.numX; ++i) {
        const gert::Shape* xShape = context->GetDynamicInputShape(GMM_INDEX_IN_X, i);
        OP_CHECK_NULL_WITH_CONTEXT(context, xShape);
        OP_CHECK_IF(xShape->GetDim(1) != weightKDimValue,
                  OP_LOGE(context->GetNodeName(), "k dim value of x[%zu] is not matched with weight.", i),
                  return GRAPH_FAILED);
        OP_CHECK_IF(innerAxisDimId > -1 && xShape->GetDim(innerAxisDimId) > GMM_MAX_INNER_AXIS,
                  OP_LOGE(context->GetNodeName(), "inner axis size of x[%zu] is larger than %ld!", i, GMM_MAX_INNER_AXIS),
                  return GRAPH_FAILED);
    }
    innerAxisDimId = !transposeWeight ? 2 : -1;  // If w is not transposed, check N(2) axis; otherwise, check k axis, which can be skipped
    ...
    OP_CHECK_IF(CheckWeightShapeInnerAxisEven(context, paramsInfo.numWeight, 2) != GRAPH_SUCCESS,
              ...
              return GRAPH_FAILED);
    // layered: every layer's expert count must match; all_* lists share the layer count.
    for (size_t i = 1; i < paramsInfo.numWeight; ++i) {
        const gert::Shape* wShape = context->GetDynamicInputShape(GMM_INDEX_IN_WEIGHT, i);
        OP_CHECK_NULL_WITH_CONTEXT(context, wShape);
        OP_CHECK_IF(wShape->GetDim(0) != static_cast<int64_t>(paramsInfo.groupNum),
                  OP_LOGE(context->GetNodeName(), "all_weight[%zu] expert dim [%ld] differs from all_weight[0] [%zu]; "
                            "every layer must cover the same expert set.", i, wShape->GetDim(0), paramsInfo.groupNum),
                  return GRAPH_FAILED);
    }
    // check groupList
...
// layered: the separated-weight split-m checkers were removed - the weight list
// length is the layer count and every element is a full-expert single tensor,
// so only the single-weight case exists.

static ge::graphStatus CheckCaseSplitM(gert::InferShapeContext* context, bool transposeWeight,
                                       const GMMParamsInfo& paramsInfo) {
    // layered: weight list length is the layer count, not group separation; every
    // element is a single full-expert (e, k, n) tensor, so all cases route through
    // the single-weight checker (mirrors tiling's isSingleWeight_ = true).
...
IMPL_OP_INFERSHAPE(GroupedMatmulLayered)
