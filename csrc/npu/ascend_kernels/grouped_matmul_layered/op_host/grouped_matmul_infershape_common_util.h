...
constexpr size_t GMM_INDEX_IN_LAYER = 4UL;
constexpr size_t GMM_INDEX_IN_GROUP_LIST = 5UL;
constexpr size_t GMM_INDEX_IN_PERTOKEN_SCALE = 6UL;
// Inputs dropped in the layered migration (offset / antiquant_scale / antiquant_offset).
// Kept as out-of-range sentinels so the A16-antiquant infershape checkers still compile;
// those code paths are unreachable with the trimmed dtype columns (x INT8 only).
constexpr size_t GMM_INDEX_IN_OFFSET = 99UL;
constexpr size_t GMM_INDEX_IN_ANTIQUANT_SCALE = 99UL;
constexpr size_t GMM_INDEX_IN_ANTIQUANT_OFFSET = 99UL;
