...
public:
    ...
    struct Params {
        ...
        Params(uint32_t m_, uint32_t k_, uint32_t n_, uint32_t problemCount_, uint32_t workspaceStages_,
               ...
               int64_t curLayer_, uint32_t layerNum_)
            : m(m_), k(k_), n(n_), problemCount(problemCount_), workspaceStages(workspaceStages_),
              ...
              ptrWorkspace(ptrWorkspace_), curLayer(curLayer_), layerNum(layerNum_)
        {
        }
        int64_t curLayer = 0;  // layered: current layer index for weight/scale
        uint32_t layerNum = 1;  // layered: all_* list length, bounds-checks curLayer
    };
    ...
    CATLASS_DEVICE void operator()<AscendC::AIC>(Params const &params)
    {
        ...
        for (uint32_t groupIdx = 0; groupIdx < params.problemCount; ++groupIdx) {
            ...
            gmB.SetGlobalBuffer(GetLayerTensorAddr<ElementB>(params.curLayer, params.layerNum, params.ptrB) + gmGroupOffsetB);
    ...
    CATLASS_DEVICE void operator()<AscendC::AIV>(Params const &params)
    {
        ...
        uint32_t kNum = CeilDiv(params.k, BlockMmad::L1A_K_ONE_TIME);
        uint32_t stageId = 0;
        auto ptrScaleBegin = GetLayerTensorAddr<ElementScale>(params.curLayer, params.layerNum, params.ptrScale);
