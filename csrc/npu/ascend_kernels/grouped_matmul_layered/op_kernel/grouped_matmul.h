...
class GMMCompute {
 ...
 protected:
    ...
    GM_ADDR yTensorPtr;
    int64_t curLayer_ = 0;  // layered: current layer index for all_weight/all_bias/all_scale
    uint32_t layerNum_ = 1;  // layered: all_* list length, bounds-checks curLayer_
    GlobalTensor<AT> xGm;
...
__aicore__ inline void GMMCompute<mmType, sync>::Init(GM_ADDR x, GM_ADDR weight, GM_ADDR bias, GM_ADDR scale,
                                                      ...
                                                      GM_ADDR workspace, int64_t curLayer,
                                                      const GMMBaseParams* __restrict gmmBaseParams,
                                                      const TCubeTiling* __restrict mmTilingData,
                                                      TPipe* tPipe) {
    xTensorPtr = x;
    ...
    yTensorPtr = y;
    curLayer_ = curLayer;
    layerNum_ = gmmBaseParams->layerNum;  // layered: bounds for the device-side layer index
    pipe = tPipe;
...
__aicore__ inline void GMMCompute<mmType, sync>::SetGlobalBufferBias(uint32_t groupIdx,
    ...
    if (hasBias) {
        {
            // layered: all_bias resolved to the current layer; expert offset within the layer
            biasGm.SetGlobalBuffer(GetLayerTensorAddr<BiasT>(curLayer_, layerNum_, biasTensorPtr) + mnConfig.nAxisBaseOffset);
        }
#if defined(__CCE_AICORE__) && __CCE_AICORE__ == 220
        constexpr bool isBiasEpilogue_ =
            AscendC::IsSameType<DTYPE_ALL_BIAS, bfloat16_t>::value &&
            (AscendC::IsSameType<DTYPE_ALL_SCALE, bfloat16_t>::value ||
             AscendC::IsSameType<DTYPE_ALL_SCALE, float>::value);
...
__aicore__ inline GlobalTensor<typename mmType::BT::T> GMMCompute<mmType, sync>::SetGlobalBufferW(
    ...
    if (isA8W4FakeQuant) {
        ...
        // layered: all_weight resolved to the current layer; expert offset within the layer
        weightGmLocal.SetGlobalBuffer(
            GetLayerTensorAddr<BT>(curLayer_, layerNum_, weightTensorPtr) + mnConfig.wBaseOffset + wOffset);
    }
...
__aicore__ inline void GMMCompute<mmType, sync>::MMCompute(uint32_t groupIdx, MNConfig& mnConfig, uint32_t coreIdx) {
...
#if defined(GMM_QUANT_INT8)
    {
        ...
        scaleGm.SetGlobalBuffer(
            GetLayerTensorAddr<DTYPE_ALL_SCALE>(curLayer_, layerNum_, scaleTensorPtr) + mnConfig.nAxisBaseOffset);
    }
