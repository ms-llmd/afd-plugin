...
public:
    ...
    // layered: weight and scale are per-layer inputs (one list element per layer), so the
    // pre-quantization pass must read the current layer rather than element 0. Its output
    // lands in workspace and is consumed as a single tensor by the main compute, which means
    // a wrong layer here cannot be corrected downstream.
    __aicore__ inline void Init(GM_ADDR weight, GM_ADDR y, GM_ADDR scale, GM_ADDR workspace,
                                int64_t curLayer, uint32_t layerNum, const GMMBaseParams& tilingData,
                                TPipe *pipe);
...
template <CubeFormat wFormat>
__aicore__ inline void GMMA8W4FakeQuantPreProcess<wFormat>::Init(GM_ADDR weight, GM_ADDR y, GM_ADDR scale,
        GM_ADDR workspace, int64_t curLayer, uint32_t layerNum, const GMMBaseParams& tilingData, TPipe *pipe){
    this->tiling = &tilingData;
    weightGm.SetGlobalBuffer(GetLayerTensorAddr<int4b_t>(curLayer, layerNum, weight));
    yGm.SetGlobalBuffer((__gm__ int8_t *)workspace);
    if (tiling->quantGroupNum == 1) {   //per channel
        this->isPerchannel = true;
        scaleGm.SetGlobalBuffer(GetLayerTensorAddr<int64_t>(curLayer, layerNum, scale));
