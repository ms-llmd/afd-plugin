...
#if defined(GMM_ANTI_QUANT) && defined(ORIG_DTYPE_ALL_WEIGHT) && defined(DT_INT8) && \
    ORIG_DTYPE_ALL_WEIGHT == DT_INT8
...
__aicore__ inline void GMMA16W8MSDProcess<ComputeType>::SetMNConfigs(
    ...
    // 2: according to experiments when n larger than or equal 2k, singleN 1024 has better performance
    // 1024: larger singleN can reduce preprocess iter num than reduce syncall nums and has better performance
    // 4: when satisfy requirements of different singleN, singleN should be quarter of n align up to 1024
...
__aicore__ inline void GMMA16W8MSDCompute<mmType, sync>::Init(GM_ADDR x, GM_ADDR weight, GM_ADDR bias,
    ...
    if (hasBias) {
        biasGm.SetGlobalBuffer(GetTensorAddr<DTYPE_ALL_BIAS>(0, bias));
    }
    InitLocalTensor();
...
__aicore__ inline void GMMA16W8MSDCompute<mmType, sync>::PreProcess(
    ...
    // larger than half of matmul baseM, this group should split in preprocess, and should get group num after
    // split first.
    for (uint32_t gIdx = 0; gIdx < preValidGroupCount; ++gIdx) {
        // ensure each group after split has at least one core when preprocessing.
