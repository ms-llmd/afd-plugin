...
public:
    ...
    __aicore__ inline GMMA4W8AutotilingCompute(GM_ADDR addrA, GM_ADDR addrB, GM_ADDR addrC, GM_ADDR addrGrpList,
                                               ...
                                               GM_ADDR addrWorkspace, int64_t curLayer, A8W4HPTiling *tilingData,
                                               TPipe *p)
        : pipe(p), gmA(addrA), gmB(addrB), gmC(addrC), gmGrpListOptional(addrGrpList), gmBias(addrBias),
          gmOffset(addrOffset), gmSa(addrSa), gmSw(addrSw), gmWorkspaceDevice(addrWorkspace), curLayer_(curLayer),
    ...
    __aicore__ inline void Init()
    {
        ...
        // layered: all_weight/all_bias/all_scale resolved to the current layer
        gmB = GetLayerTensorAddr<uint8_t>(curLayer_, tiling_data->layerNum, gmB);
        gmC = GetTensorAddr<uint8_t>(0, gmC);
        gmBias = GetLayerTensorAddr<uint8_t>(curLayer_, tiling_data->layerNum, gmBias);
        gmOffset = GetTensorAddr<uint8_t>(0, gmOffset);
        gmSw = GetLayerTensorAddr<uint8_t>(curLayer_, tiling_data->layerNum, gmSw);
    }
