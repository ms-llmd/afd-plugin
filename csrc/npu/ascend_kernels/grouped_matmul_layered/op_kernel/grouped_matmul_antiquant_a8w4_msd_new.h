...
public:
    ...
    __aicore__ inline void Init(GM_ADDR x, GM_ADDR weight, GM_ADDR bias,
        ...
        GM_ADDR y, GM_ADDR workspace, int64_t curLayer, const GMMBaseParams* tilingData,
        ...
        const TCubeTiling* mmTilingData, TPipe *tPipeIn)
{
    ...
    weightGm.SetGlobalBuffer(GetLayerTensorAddr<DTYPE_WEIGHT_DEV_A8W4MSD_NEW>(curLayer, tiling->layerNum, weight));
    biasGm.SetGlobalBuffer(GetLayerTensorAddr<DTYPE_BIAS_A8W4MSD_NEW>(curLayer, tiling->layerNum, bias));
    mmOutGm.SetGlobalBuffer(reinterpret_cast<__gm__ cT::T *>(workspace));
    scaleGm.SetGlobalBuffer(GetLayerTensorAddr<DTYPE_SCALE_DEV_A8W4MSD_NEW>(curLayer, tiling->layerNum, scale));
