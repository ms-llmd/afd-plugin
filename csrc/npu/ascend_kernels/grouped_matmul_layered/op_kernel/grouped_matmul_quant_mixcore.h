...
class GMMQuantMixCoreCompute : public GMMCompute<mmType, sync> {
 public:
    ...
    __aicore__ inline void Init(GM_ADDR x, GM_ADDR weight, GM_ADDR bias, GM_ADDR scale, GM_ADDR offset,
                                ...
                                GM_ADDR perTokenScale, GM_ADDR y, GM_ADDR workspace, int64_t curLayer,
 ...
 private:
    ...
    GlobalTensor<DTYPE_ALL_SCALE> scaleGm;
    GlobalTensor<float> perTokenScaleGm;
    GlobalTensor<CT> mmOutGm;
    GlobalTensor<DTYPE_ALL_BIAS> biasGm;
    // define the que
    ...
    LocalTensor<DTYPE_ALL_SCALE> scaleInUb;
    LocalTensor<DTYPE_ALL_BIAS> biasLocal;
    ...
    static constexpr bool isBiasEpilogue_ =
        AscendC::IsSameType<DTYPE_ALL_BIAS, bfloat16_t>::value &&
        (AscendC::IsSameType<DTYPE_ALL_SCALE, bfloat16_t>::value || AscendC::IsSameType<DTYPE_ALL_SCALE, float>::value);
};
...
__aicore__ inline void GMMQuantMixCoreCompute<mmType, sync>::Init(GM_ADDR x, GM_ADDR weight, GM_ADDR bias,
    ...
    this->GMMCompute<mmType, sync>::Init(x, weight, bias, scale, offset, antiquantScale, antiquantOffset, groupList,
        perTokenScale, y, workspace, curLayer, gmmBaseParams, mmTilingData, tPipe);
...
__aicore__ inline void GMMQuantMixCoreCompute<mmType, sync>::SetPerTokenQuantStaticBuffer(
    ...
    if ASCEND_IS_AIV {
        ...
        this->pipe->InitBuffer(scaleInQueue, 2, baseN_ * sizeof(DTYPE_ALL_SCALE));
        if (isPerTokenQuant) {
            // 2: enabling double buffer, occupying two buffer.
            this->pipe->InitBuffer(perTokenScaleInQueue, 2, baseM_ * sizeof(float));
            if constexpr (isBiasEpilogue_) {
                this->pipe->InitBuffer(biasInQueue, 2, baseN_ * sizeof(DTYPE_ALL_BIAS));
            }
        }
        // 2: enabling double buffer, occupying two buffer.
...
__aicore__ inline void GMMQuantMixCoreCompute<mmType, sync>::SetPerTokenQuantRefreshedBuffer(const MNConfig mnConfig) {
    ...
    if (this->GMMCompute<mmType, sync>::isA8W4FakeQuant == true) {
        scaleGm.SetGlobalBuffer((__gm__ DTYPE_ALL_SCALE *)scaleTensorPtr + mnConfig.nAxisBaseOffset);
    } else {
        // layered: all_scale resolved to the current layer; expert offset within the layer
        scaleGm.SetGlobalBuffer(
            GetLayerTensorAddr<DTYPE_ALL_SCALE>(this->curLayer_, this->layerNum_, scaleTensorPtr) + mnConfig.nAxisBaseOffset);
    }
    if (isPerTokenQuant) {
        perTokenScaleGm.SetGlobalBuffer((__gm__ float *)perTokenScaleTensorPtr + mnConfig.mAxisBaseOffset);
        if constexpr (isBiasEpilogue_) {
            biasGm.SetGlobalBuffer(
                GetLayerTensorAddr<DTYPE_ALL_BIAS>(this->curLayer_, this->layerNum_, biasTensorPtr) + mnConfig.nAxisBaseOffset);
        }
    }
...
{
    // GM copy scale
    DataCopyPadExtParams<DTYPE_ALL_SCALE> padParams;
    DataCopyExtParams scaleParams;
    scaleParams.blockLen = curBaseN * sizeof(DTYPE_ALL_SCALE);
    scaleParams.blockCount = 1;
    scaleParams.srcStride = 0;
    scaleParams.dstStride = 0;
    LocalTensor<DTYPE_ALL_SCALE> scaleLocal = scaleInQueue.AllocTensor<DTYPE_ALL_SCALE>();
    DataCopyPad(scaleLocal, scaleGm[scaleOffset], scaleParams, padParams);
    scaleInQueue.EnQue(scaleLocal);

    scaleInUb = scaleInQueue.DeQue<DTYPE_ALL_SCALE>();
...
__aicore__ inline void GMMQuantMixCoreCompute<mmType, sync>::DataCopyBias(uint32_t curBaseN, uint32_t alignBaseN,
                                                                          uint64_t biasOffset)
{
    DataCopyPadExtParams<DTYPE_ALL_BIAS> padParams;
    DataCopyExtParams biasParams;
    biasParams.blockLen = curBaseN * sizeof(DTYPE_ALL_BIAS);
    biasParams.blockCount = 1;
    biasParams.srcStride = 0;
    biasParams.dstStride = 0;
    LocalTensor<DTYPE_ALL_BIAS> biasLocal = biasInQueue.AllocTensor<DTYPE_ALL_BIAS>();
    DataCopyPad(biasLocal, biasGm[biasOffset], biasParams, padParams);
    biasInQueue.EnQue(biasLocal);
    biasLocal = biasInQueue.DeQue<DTYPE_ALL_BIAS>();
